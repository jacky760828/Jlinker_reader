...
namespace JLinkSDK {
  ...
  public class JLink {
    ...
    [DllImport(sDLL, EntryPoint = "JLINK_OpenEx",            CallingConvention = CallingConvention.StdCall)] unsafe private static extern IntPtr JLINK_OpenEx            (JLINK_LOG pfLog, JLINK_LOG pfErrorOut);
    ...
    */
    public unsafe static string OpenEx(JLINK_LOG pfLog, JLINK_LOG pfErrorOut) {
      ...
      //
      p      = JLINK_OpenEx(pfLog, pfErrorOut);
      sError = Marshal.PtrToStringAnsi(p); // p is actually a pointer to an immutable string, so we have to convert it.
      return sError;
    }
